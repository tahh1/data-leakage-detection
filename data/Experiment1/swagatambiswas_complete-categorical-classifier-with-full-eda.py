...
# # **Introduction**
# 
# This is a binary classification model. The data used here is presented by a telecom company having customer data to predict the **Churn** rate of customers.
# The notebook contains complete **data analysis** to understand the data and how it affects customer churn, then comes **data pre-processing** before **training multiple models** to get the best parameters/model for prediction.
# 
# These methods can be applied to any other kind of data (just modify it to your needs). 
# 
# **And if you like my work please upvote this Kernel. This encourages or motivates people like me, who contributes to Kaggle on their own time with the intention to share & gain knowledge, to continue the effort. Furthermore, if I made a mistake or can do something more, please leave a comment in the comments section to help me out. Many thanks in advance!**
...
telcom = pd.read_csv("/kaggle/input/telecom/Hiring_Challenge/Telecom/Churn.csv")
#first few rows
telcom.head()
...
Id_col     = ['customerID']
target_col = ["Churn"]
cat_cols   = list(telcom.nunique()[telcom.nunique() < 6].keys()).tolist()
...
#labels
lab = list(telcom["Churn"].value_counts().keys()).tolist()
#values
...
def plot_pie(column) :
    ...
    trace1 = go.Pie(values  = churn[column].value_counts().values.tolist(),
                    labels  = list(churn[column].value_counts().keys()).tolist(),
    ...
    trace2 = go.Pie(values  = not_churn[column].value_counts().values.tolist(),
                    labels  = list(not_churn[column].value_counts().keys()).tolist(),
                    hoverinfo = "label+percent+name",
                    marker  = dict(line = dict(width = 2,
                                               color = "rgb(243,243,243)")
                                  ),
...
bin_cols   = list(telcom.nunique()[telcom.nunique() == 2].keys()).tolist()
...
bi_cs = list(telcom.nunique()[telcom.nunique() == 2].keys())
...
for i,j,k in itertools.zip_longest(lst,list(range(length)),mods) :
    plt.subplot(4,3,j+1)
