...
def feature_engineering(dataset):
    ...
    # de leeftijdsgroepen.
    dataset['AgeGroup'], agebins = pd.cut(dataset['Age'], 6, labels=list(range(6)), retbins=True)
...
sns.barplot(x = list(X.keys()), y = DTree.feature_importances_, alpha = alpha)
...
plt.figure(figsize = (18, 7))
sns.barplot(x = list(X.keys()), y = ETree_grid.feature_importances_, alpha = alpha)
...
plt.figure(figsize = (18, 7))
sns.barplot(x = list(X.keys()), y = RandomForest_grid.feature_importances_, alpha = alpha)
...
scores = {key : val.mean() for key, val in list(model_scores.items())}
